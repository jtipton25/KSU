...
```{r}
knitr::include_graphics(here("2018", "ksu", "images", "Science.png"))
```

## Introduction | Predicting the future by learning from the past

```{r}
knitr::include_graphics(here("2018", "ksu", "images", "past1.png"))
```

## Introduction | Predicting the future by learning from the past

```{r}
knitr::include_graphics(here("2018", "ksu", "images", "past2.png"))
```

## Introduction | Predicting the future by learning from the past

```{r, out.width="50%"}
knitr::include_graphics(here("2018", "ksu", "images", "future.png"))
```
...
knitr::include_graphics(here("2018", "ksu", "images", "pollen.png"))
```
...
```{r, out.width='95%', echo=FALSE}
knitr::include_graphics(here("2018", "ksu", "images", "pollen-linear-fit.png"))
```
...
```{r, out.width='95%', echo=FALSE}
knitr::include_graphics(here("2018", "ksu", "images", "pollen-fit.png"))
```
...
```{r, out.width='95%', echo=FALSE}
knitr::include_graphics(here("2018", "ksu", "images", "pollen-simulation.png"))
```
...
dat <- data.frame( 
  ...
  lower_ci   = c(NA, apply(Z_post_mean, 2, mean) -
    2 * apply(Z_post_mean, 2, sd) / sqrt(nrow(Z_post_mean))),
  upper_ci   = c(NA, apply(Z_post_mean, 2, mean) + 
    2 * apply(Z_post_mean, 2, sd) / sqrt(nrow(Z_post_mean))))

ggplot(data = dat, aes(x = year, y = climate)) +
   geom_line() + 
  geom_line(aes(x = year, y = estimate), color = "red") +
  geom_ribbon(aes(ymin = lower_ci, ymax = upper_ci), alpha = 0.25, fill="red") +
...
```{r, fig.show='animate', cache=TRUE, message=FALSE, warning=FALSE}
...
Z_post_lower_50 <- apply(Z_post, c(2, 3), quantile, prob=0.25)
Z_post_upper_50 <- apply(Z_post, c(2, 3), quantile, prob=0.75)
Z_post_lower_95 <- apply(Z_post, c(2, 3), quantile, prob=0.025)
Z_post_upper_95 <- apply(Z_post, c(2, 3), quantile, prob=0.975)

dat <- data.frame(
  Z        = c(apply(cbind(c(data$Z0 * sd_X + mu_X), Z_post_mean), 2, mean)),
  lower_50 = c(apply(cbind(NA, Z_post_lower_50), 2, mean)),
  upper_50 = c(apply(cbind(NA, Z_post_upper_50), 2, mean)),  
  lower_95 = c(apply(cbind(NA, Z_post_lower_95), 2, mean)),
  upper_95 = c(apply(cbind(NA, Z_post_upper_95), 2, mean)), 
  year     = 0:tt*250)
